package com.timefusion.jfxcalendar.views;

import com.timefusion.jfxcalendar.model.CalendarEvent;
import com.timefusion.jfxcalendar.model.CalendarEventManager;
...
public class CalendarMonthView extends CalendarView {
  ...
  public void refreshCalendar(LocalDate selectedDate) {
    ...
    for (int i = 1; i <= firstDayIndex; i++) {
      ...
      ObservableList<CalendarEvent> eventList = eventManager.getEventsOn(
        currentDayDate
      );
  ...
  private Node createCustomPane(
    ...
    double extraSpacing =
      eventRootPane.getPadding().getLeft() +
      eventRootPane.getPadding().getRight() +
      vGap *
      2;
  ...
  private VBox createResizableBox(
    VBox eventRootPane,
    double padding,
    int eventCount,
    int priority
